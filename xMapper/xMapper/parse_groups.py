...
def parse_groups(xml_string):
    ...
    for groups in groups_parent.findall("model"):
        group_data = {attr: groups.get(attr) for attr in groups.attrib}  # Store attributes
        # active = model.find("Active")
        if ('Active' in group_data):
            print("Skipping inactive ", group_data['name'])
            continue

        print("Working on ....", group_data['name'])
...
if __name__ == "__main__":
    ...
    groups_xml = read_xml_file(xml_data)
